...
class App extends Component {
    constructor(props) {
        ...
        let userID = 'bryce1';
        const indexCheckpoints = [];
        const blockIndicesToGet = [];
        ...
        .then(documentProgress => {
            if (documentProgress.length >= 1) {
                ...
                indexCheckpoints = TextEngine.getIndexCheckpoints(mostRecentDocumentProgress.document, 500);
                blockIndicesToGet = TextEngine.getIndicesFromCheckpoints(indexCheckpoints, mostRecentDocumentProgress.currentBlock);
                return [mostRecentDocumentProgress.document, blockIndicesToGet];
            } else {
                return new Error('Error: No documents found in user progress');
            }
        })
    ...
    goPrev() {
        ...
        .then(returnedBlocks => {
            UserEngine.updateUserDocumentProgress(this.state.userID, this.state.documentID, this.returnedBlocks[0].index);
        })
        .catch(err => console.log(err));
    }
    goNext() {
