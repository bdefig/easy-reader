...
import * as UserEngine from './UserEngine.js';
...
class App extends Component {
    constructor(props) {
        ...
        // TODO: Fetch document metadata from server
        ...
        // TODO: Deal with user ID better
        let userID = '59f3be1f0a510c84f9acd76a';
        ...
        UserEngine.getUserDocumentProgressByUserID(this.state.userID)
        .then(documentProgress => {
            if (documentProgress.length >= 1) {
                const mostRecentDocumentProgress = documentProgress[0];
                const indexCheckpoints = TextEngine.getIndexCheckpoints(mostRecentDocumentProgress.document, 500);
                const blockIndicesToGet = TextEngine.getIndicesFromCheckpoints(indexCheckpoints, mostRecentDocumentProgress.currentBlock);
                this.setState({
                ...
                const indexCheckpoints = TextEngine.getIndexCheckpoints(firstDocument, 500);
                // const blockIndicesToGet = TextEngine.getIndicesFromCheckpoints(indexCheckpoints, 0);
                this.setState({
                    indexCheckpoints: indexCheckpoints
                });
                return [firstDocument, [0, 4]];
            }
        })
        .then(docAndIndicesToGet => {
            const documentMetadata = docAndIndicesToGet[0];
            const lowerIndex = docAndIndicesToGet[1][0];
            const higherIndex = docAndIndicesToGet[1][1];
            return TextEngine.getBlocksByIndices(documentMetadata, lowerIndex, higherIndex);
        })
        .then(returnedBlocks => {
            this.setState({
    ...
    goPrev() {
        if (this.state.currentFirstIndex > 0) {
            ...
            .then(returnedBlocks => {
                ...
                UserEngine.updateUserDocumentProgress(this.state.userID, this.state.currentDocumentMetadata._id, returnedBlocks[0].index);
            })
            .catch(err => console.log(err));
        }
    }
